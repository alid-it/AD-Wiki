...
/** Encrypts integration credentials with AES-256-GCM and a deployment-owned key. */
@Injectable()
export class IntegrationEncryptionService {
  ...
  decrypt(value: string): string {
    try {
      ...
      throw new InternalServerErrorException("Gespeicherte Integrationsdaten konnten nicht entschlüsselt werden.");
    }
  }

  private key(): Buffer {
    const raw = process.env.INTEGRATION_ENCRYPTION_KEY?.trim();
    if (!raw) throw new Error("INTEGRATION_ENCRYPTION_KEY fehlt.");
    const key = Buffer.from(raw, "base64");
    if (key.length !== 32 || key.toString("base64") !== raw) {
      throw new Error("INTEGRATION_ENCRYPTION_KEY muss 32 Byte Base64 enthalten.");
    }
    return key;
  }
}
